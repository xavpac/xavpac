  ...
  useEffect(() => {
    ...
    async function loadWeather() {
      ...
      try {
        ...
        if (!response.ok) throw new Error(`Réponse météo ${response.status}`);
      ...
      <section className="weather-layout">
            ...
            <div className="forecast-grid">
              {weather.daily.time.map((day, index) => (
                <article key={day} className="forecast-day">
                  ...
                  <div><b>{Math.round(weather.daily.temperature_2m_max[index])}°</b><span>{Math.round(weather.daily.temperature_2m_min[index])}°</span></div>
        ...
        <aside className="weather-side">
          ...
          <article className="panel weather-update">
            ...
            <h3>{updatedAt || "—"}</h3>
            <p className="muted">Actualisation automatique toutes les dix minutes et lors des déplacements.</p>
          </article>
        </aside>
      </section>
    </>
